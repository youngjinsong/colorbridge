...
<tr align=center height=29><td class=n><?=$loop_number?></td>

  <?=$hide_cart_start?><td><input type=checkbox name=cart value="<?=$data[no]?>"></td><?=$hide_cart_end?>

  <td align=left style='word-break:break-all;'><?=$subject?><font class=n>

<?
$comment_num=" <font color=#656565>".$data[total_comment]."";
if($data[total_comment]==0) {
  $comment_num="";
}
echo "$comment_num";?> <?=$insert?><?=$icon?></font></td> 

  <td nowrap></div></td>
<td align=right><font class=n><?=$date=date("Ymd",$data[reg_date])?>&nbsp;</font></td>
</tr>
