...
//The slideshow starts when all the images are loaded
	...
	function nextSlide(nav){    //funzione per il fading delle immagini
		elem.addClass('diaposliding');	//aggiunge una classe che mi dice che il fading 챔 in corso
		
		var vis = parseFloat($('> '+opts.selector +'.diapocurrent',elem).not('#pix_canvas').not('#pix_canvas_wrap').not('#pix_next').not('#pix_prev').not('#pix_commands').index());    //la variabile 챔 il numero del div partendo da 0

		if(nav>0){    //se siamo all'ultimo div o se ancora non ho creato nessun div
			var i = nav-1;
		} else if (vis == amountSlide-1) { 
			var i = 0;
		} else {    //altrimenti l'indice 챔 l'id corrent pi첫 uno, quindi il div successivo
			var i = vis+1;
		}
